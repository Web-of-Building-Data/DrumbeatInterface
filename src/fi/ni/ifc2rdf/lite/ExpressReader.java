...
import fi.ni.ifc2rdf.lite.vo.AttributeVO;
import fi.ni.ifc2rdf.lite.vo.EntityVO;
import fi.ni.ifc2rdf.lite.vo.InverseVO;
import fi.ni.ifc2rdf.lite.vo.PropertyVO;
import fi.ni.ifc2rdf.lite.vo.TypeVO;
...
public class ExpressReader {
	...
	try {
		...
		if (evo.getSuperclass() == null)
		    ...
		    out.write("ifc:" + evo.getName() + " a rdfs:Class; rdfs:subClassOf ifc:" + evo.getSuperclass() + ".\n");

		for (int n = 0; n < evo.getAttributes().size(); n++) {
		    String property = evo.getAttributes().get(n).getName().toLowerCase();// formatProperty(evo.getAttributes().get(n).getName());
		    ...
		    out.write("ifc:" + property + " rdfs:domain ifc:" + pairs.getValue().getName() + ".\n");
		}

		for (int n = 0; n < evo.getInverses().size(); n++) {
		    String property = formatProperty(evo.getInverses().get(n).getName());
		    PropertyVO t = properties.get(property);
		    if (t == null) {
	...
	try {
		...
		tmp = ToolkitString.strReplaceLike(tmp, "<_NAME_>", property.getName() + "_enumeration");
		tmp = ToolkitString.strReplaceLike(tmp, "<_ENUM_LIST_>", property.getEnumerationsAsString());
		out.write(tmp);

		tmp = type_object_property1;
		tmp = ToolkitString.strReplaceLike(tmp, "<_NAME_>", property.getName());
		tmp = ToolkitString.strReplaceLike(tmp, "<_DOMAIN_>", property.getIfcClassesAsString());
		tmp = ToolkitString.strReplaceLike(tmp, "<_RANGE_>", "ifc:" + property.getName() + "_enumeration");
		out.write(tmp);
		...
		if (property.isEntity) {
		    ...
		    tmp = ToolkitString.strReplaceLike(tmp, "<_NAME_>", property.getName() + "_list");
		    tmp = ToolkitString.strReplaceLike(tmp, "<_RANGE_>", "ifc:" + property.getRawtype());
		    out.write(tmp);

		    tmp = type_object_property1;
		    tmp = ToolkitString.strReplaceLike(tmp, "<_NAME_>", property.getName());
		    tmp = ToolkitString.strReplaceLike(tmp, "<_DOMAIN_>", property.getIfcClassesAsString());
		    tmp = ToolkitString.strReplaceLike(tmp, "<_RANGE_>", "ifc:" + property.getName() + "_list");
		    out.write(tmp);
		    ...
		    tmp = ToolkitString.strReplaceLike(tmp, "<_NAME_>", property.getName() + "_list");
		    tmp = ToolkitString.strReplaceLike(tmp, "<_DATATYPE_>", property.getRawtype());
		    out.write(tmp);
	...
	try {
		...
		for (int n = 0; n < evo.getAttributes().size(); n++) {
		    ...
		    PropertyVO t = properties.get(property);
		    if (t == null) {
			EntityVO entity = this.entities.get(formatClassName(evo.getAttributes().get(n).getType().getName()));

			String primary_type = evo.getAttributes().get(n).getType().getPrimarytype();
			if (primary_type.equals("ENUMERATION")) {
			    t = new PropertyVO(property, evo.getAttributes().get(n).isSet(), false, null, true, evo.getAttributes().get(n).getType().getEnum_entities());
			} else {
			    ...
			    if (entity != null)
				t = new PropertyVO(property, evo.getAttributes().get(n).isSet(), true, evo.getAttributes().get(n).getType().getName());
			    else
				t = new PropertyVO(property, evo.getAttributes().get(n).isSet(), false, xsd_type);
			}
		...
		for (int n = 0; n < evo.getInverses().size(); n++) {
		    ...
		    PropertyVO t = properties.get(property);
		    if (t == null) {
			t = new PropertyVO(property, true, true, evo.getInverses().get(n).getIfc_class());
			properties.put(property, t);
		    }
		    t.addIfcClass(evo.getName());
		}
	...
	while (it.hasNext()) {
	    ...
	    EntityVO evo = pairs.getValue();
	    if (evo.getSuperclass() != null)
		System.out.println(pairs.getKey() + " s:" + evo.getSuperclass());
	    else
		System.out.println(pairs.getKey());

	    for (int n = 0; n < evo.getDerived_attribute_list().size(); n++) {
		if (evo.getDerived_attribute_list().get(n).isReverse_pointer())
		    System.out.println(" " + evo.getDerived_attribute_list().get(n).getType().getName() + ":" + evo.getDerived_attribute_list().get(n).getType().getPrimarytype() + "   " + evo.getDerived_attribute_list().get(n).getName() + " CAN BE REVERSED");
		else
		    System.out.println(" " + evo.getDerived_attribute_list().get(n).getType().getName() + ":" + evo.getDerived_attribute_list().get(n).getType().getPrimarytype() + "   " + evo.getDerived_attribute_list().get(n).getName());
	    }
	    for (int n = 0; n < evo.getDerived_inverse_list().size(); n++) {
		System.out.println(" i:" + evo.getDerived_inverse_list().get(n).getName());
	    }
	}
    }
	...
	try {
	    BufferedWriter out = new BufferedWriter(new FileWriter("c:\\jo\\generated\\" + interface_name + "_StringValue.java"));
	    ...
	    out.write(interface_name + "_StringValue implements " + interface_name);
	...
	while (it.hasNext()) {
	    ...
	    try {
		BufferedWriter out = new BufferedWriter(new FileWriter("c:\\jo\\interfaces\\" + pairs.getValue() + ".java"));
	...
	while (it.hasNext()) {
	    ...
	    EntityVO evo = pairs.getValue();
	    try {
		BufferedWriter out = new BufferedWriter(new FileWriter("c:\\jo\\generated\\" + evo.getName() + ".java"));
		...
		for (int n = 0; n < evo.getInverses().size(); n++) {
		    out.write(" public InverseLinksList<" + evo.getInverses().get(n).getIfc_class() + "> get" + formatGetterANDSetter(evo.getInverses().get(n).getName()) + "() {\n");
		    out.write("   return " + evo.getInverses().get(n).getName() + ";\n");
		    out.write("\n }\n");
		    out.write(" public void set" + formatGetterANDSetter(evo.getInverses().get(n).getName()) + "(" + evo.getInverses().get(n).getIfc_class() + " value){\n");
		    out.write("   this." + evo.getInverses().get(n).getName() + ".add(value);\n");
		    out.write("\n }\n\n");
		}
     ...
     */
    private void outputJavaAttributes(EntityVO evo, BufferedWriter out) throws IOException {
	...
	for (int n = 0; n < evo.getAttributes().size(); n++) {
	    ...
	    if ("INTEGER".equals(primary_type))
		java_type = "Long";
	    if (evo.getAttributes().get(n).getType().getName().equalsIgnoreCase("IfcTimeStamp"))
		java_type = "Date";
	    ...
	    if (entity == null) {
		String typekey=formatClassName(evo.getAttributes().get(n).getType().getName());
		String alias=interface_aliases.get(evo.getAttributes().get(n).getType().getName());
		if(alias!=null)
		...
		if (itype != null) {
			...
			out.write(" List<" + itype + "> " + evo.getAttributes().get(n).getName() + " = new "+listtype+"<" + itype + ">();\n");
		    else
			out.write(itype + " " + evo.getAttributes().get(n).getName() + ";\n");
		} else {
		    // BASIC ATTRIBUTE
		    if (evo.getAttributes().get(n).isSet())
			out.write(" List<" + java_type + "> " + evo.getAttributes().get(n).getName() + " = new "+listtype+"<" + java_type + ">();\n");
		    else {
			out.write(" " + java_type + " " + evo.getAttributes().get(n).getName() + ";\n");
		    }
		...
		// IFC ENTITY
		if (evo.getAttributes().get(n).isSet())
		    out.write(" List<" + evo.getAttributes().get(n).getType().getName() + "> " + evo.getAttributes().get(n).getName() + " = new "+listtype+"<" + evo.getAttributes().get(n).getType().getName() + ">();\n");
		else
		    out.write(" " + evo.getAttributes().get(n).getType().getName() + "   " + evo.getAttributes().get(n).getName() + ";\n");
	...
	for (int n = 0; n < evo.getInverses().size(); n++) {
	    out.write(" InverseLinksList<" + evo.getInverses().get(n).getIfc_class() + "> " + evo.getInverses().get(n).getName() + "= new InverseLinksList<" + evo.getInverses().get(n).getIfc_class() + ">();\n");
     ...
     *             Signals that an I/O exception has occurred.
     */
    private void outputJavaGetSetMethods2Attributes(EntityVO evo, BufferedWriter out) throws IOException {
	// Getters & setters
	if (evo.getAttributes().size() > 0)
	...
	for (int n = 0; n < evo.getAttributes().size(); n++) {
		...
		if (itype != null) {
		    ...
		    // GET
		    if (evo.getAttributes().get(n).isSet()) {
			out.write(" public List<" + evo.getAttributes().get(n).getType().getName() + "> get" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "() {\n");
			out.write("   return " + evo.getAttributes().get(n).getName() + ";");
			out.write("\n }\n");
		    } else {
			out.write(" public " + itype + " get" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "() {\n");
			out.write("   return " + evo.getAttributes().get(n).getName() + ";");
			out.write("\n }\n");
		    ...
		    if (evo.getAttributes().get(n).isSet()) {
			out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(" + itype + " value){\n");
			out.write("   this." + evo.getAttributes().get(n).getName() + ".add(value);\n");
			out.write("\n }\n\n");
		    } else {
			out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(" + itype + " value){\n");
			out.write("   this." + evo.getAttributes().get(n).getName() + "=value;\n");
			out.write("\n }\n\n");
		    }
		} else {
		    // BASIC TYPES
		    // GET
		    if (evo.getAttributes().get(n).isSet()) {
			out.write(" public List<" + java_type + "> get" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "() {\n");
			out.write("   return " + evo.getAttributes().get(n).getName() + ";");
			out.write("\n }\n");
		    } else {
			out.write(" public " + java_type + " get" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "() {\n");
			out.write("   return " + evo.getAttributes().get(n).getName() + ";");
			out.write("\n }\n");
			...
			if ("REAL".equals(primary_type)) {
			    out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(String txt){\n");
			    out.write("   List<Double> value = i.toDoubleList(txt);\n");
			    out.write("   this." + evo.getAttributes().get(n).getName() + "=value;\n");
			    out.write("\n }\n\n");
			}

			else if (evo.getAttributes().get(n).getType().getName().equalsIgnoreCase("IfcTimeStamp")) {
			    out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(String txt){\n");
			    ...
			    out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(" + java_type + " value){\n");
			    out.write("   this." + evo.getAttributes().get(n).getName() + ".add(value);\n");
			    out.write("\n }\n\n");
			}
		...
		if (evo.getAttributes().get(n).isSet()) {
		    ...
		    out.write(" public " + evo.getAttributes().get(n).getType().getName() + " get" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "() {\n");
		    out.write("   return " + evo.getAttributes().get(n).getName() + ";\n");
		    out.write("\n }\n");
		}
		// SET
		if (evo.getAttributes().get(n).isSet()) {
		    out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(" + evo.getAttributes().get(n).getType().getName() + " value){\n");
		    out.write("   this." + evo.getAttributes().get(n).getName() + ".add(value);\n");
		    out.write("\n }\n\n");
		} else {
		    out.write(" public void set" + formatGetterANDSetter(evo.getAttributes().get(n).getName()) + "(" + evo.getAttributes().get(n).getType().getName() + " value){\n");
	...
	for (int n = 0; n < evo.getAttributes().size(); n++) {
	    System.out.println("   " + evo.getName() + " , " + evo.getAttributes().get(n));
	}
	...
	for (int n = 0; n < evo.getAttributes().size(); n++) {
	    attributes.put(top.getName() + "#" + evo.getAttributes().get(n).getName(), evo.getAttributes().get(n));
	...
	if (evo.getName().equalsIgnoreCase(superclass_name))
	    return true;
	if (evo.getSuperclass() != null) {
	    EntityVO sup = entities.get(evo.getSuperclass().toUpperCase());
	    if (sup != null) {
	...
	return isSubClassOf_this(entities.get(class_name.toUpperCase()), superclass_name.toUpperCase());
	...
	for (int n = 0; n < evo.getInverses().size(); n++) {
	    AttributeVO avo = attributes.get(evo.getInverses().get(n).getIfc_class() + "#" + evo.getInverses().get(n).getProperty());
		...
		System.err.println("reverse not found:" + evo.getInverses().get(n).getIfc_class() + "#" + evo.getInverses().get(n).getProperty());
	...
	case TYPE_SWITCH:
		...
		for (Map.Entry<String, TypeVO> entry : types.entrySet()) {
			...
			if(current_type.getName().equalsIgnoreCase(vo.getSelected_entities().get(n)))
			{
			    current_type.setPrimarytype(formatClassName(txt));
			    interface_aliases.put(current_type.getName(),vo.getName());
	...
	case ENTITY_READY:
		...
		tmp_entity_type = ExpressReader.formatClassName(txt.substring(0, txt.length() - 1));
		TypeVO type = types.get(tmp_entity_type);
		if (type == null) {
		    type = new TypeVO(txt.substring(0, txt.length() - 1), "CLASS");
		}
		current_entity.getAttributes().add(new AttributeVO(tmp_entity_name, type, is_set, is_list));
		state = ENTITY_STATE;
	    }
	    break;
	...
	case ENTITY_UNIQUE_TYPE:
	    ...
	    else
	    {
	       if(!txt.contains(","))
	       {
		 String unique_attribute=ExpressReader.formatProperty(txt.substring(0, txt.length() - 1));
	...
	case ENTITY_INVERSE_FOR:
		...
		tmp_inverse_ifc_property = ExpressReader.formatProperty(txt.substring(0, txt.length() - 1));
		current_entity.getInverses().add(new InverseVO(tmp_inverse_name, tmp_inverse_ifc_class, tmp_inverse_ifc_property, is_set,tmp_inverse_is_one_valued));
		state = ENTITY_INVERSE_STATE;
	    }
	    break;
