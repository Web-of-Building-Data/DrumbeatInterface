...
import fi.ni.ifc2rdf.lite.ifc2x3.IfcProject;
import fi.ni.ifc2rdf.lite.ifc2x3.IfcRoot;
import fi.ni.ifc2rdf.lite.vo.AttributeVO;
import fi.ni.ifc2rdf.lite.vo.EntityVO;
import fi.ni.ifc2rdf.lite.vo.IFC_X3_VO;
import fi.ni.ifc2rdf.lite.vo.TypeVO;
import fi.ni.ifc2rdf.lite.vo.ValuePair;
...
public class IFC_ClassModel {
	...
	/** The DRUM prefix. */
	final static String DRUM_PREFIX = "http://drumbeat.cs.hut.fi/tomcat/marmotta/resource";

	/** The IFC predicate prefix_ name. */
	...
	private String deduceSubject(Thing pointer) {
		...
		if (IfcRoot.class.isInstance(pointer)) {
			try {
				subject = "drum:" + URLEncoder.encode(((IfcRoot) pointer).getGlobalId(),"UTF-8");
	...
	private String deduceSubject(Thing pointer, String nsabr) {
		...
		if (IfcRoot.class.isInstance(pointer)) {
			try {
				subject = nsabr + ":" + URLEncoder.encode(((IfcRoot) pointer).getGlobalId(), "UTF-8");
			} catch (UnsupportedEncodingException e) {				
				e.printStackTrace();
			}
		} else {
	...
	private Thing getLineEntry(Long line_number, String class_name) {
		...
		if (thing == null) {
			...
			Class cls = null;
			try {
				cls = Class.forName("fi.ni.ifc2rdf.lite.ifc2x3." + class_name);
		...
		for (int j = 0; j < method.length; j++) {
			try {
				...
				try {
					...
					@SuppressWarnings("rawtypes")
					Class cls = Class.forName("fi.ni.ifc2rdf.lite.ifc2x3."
			...
			for (Map.Entry<Long, IFC_X3_VO> entry : linemap.entrySet()) {
				...
				if (vo.inverse_pointer_sets.size() > 0) {
					for (Map.Entry<String, LinkedList<IFC_X3_VO>> inverse_set : vo.inverse_pointer_sets
						...
						if (vo.getGid() != null) {
							subject = "drum:" + URLEncoder.encode(filter_extras(vo.getGid()), "UTF-8");
						}
